...
- Claude reviews arrive as issue comments posted by GitHub Actions. They start
  with `## Claude Review — <persona>` and include the reviewer's methodology +
  guardrails used. Treat these as feedback that must be acknowledged before
  merge.
...
- For Claude review issue comments, reply in the issue thread (not a review
  thread) with `[claude]` and state whether you will address the feedback now
...
- If feedback requires changes:
  ...
  - Reply with the fix details and commit sha (`[claude] ...`) in the same
    place you acknowledged the feedback (issue comment for Claude reviews,
