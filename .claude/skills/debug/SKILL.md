...
description:
  ...
  Code agent logs with issue/session identifiers; use when runs stall, retry
...
- Correlate Linear issue identity to a Claude Code session quickly.
...
- Primary runtime log: `log/symphony.log`
  ...
  - Includes orchestrator, agent runner, and Claude Code session lifecycle
    logs.
...
4. Trace that `session_id` across start, stream, completion/failure, and stall
   handling logs.
5. Decide class of failure: timeout/stall, session startup failure, turn
...
3. Classify the problem:
    ...
    - Session startup: `Claude session failed ...`.
...
In Symphony, Claude Code session diagnostics are emitted into
`log/symphony.log` and keyed by `session_id` (a UUID). Read them as a
lifecycle:
