defmodule Core.WorkspaceAndConfigTest do
  ...
  test "workspace remove continues when before_remove hook times out" do
    previous_timeout = Application.get_env(:core, :workspace_hook_timeout_ms)

    on_exit(fn ->
      if is_nil(previous_timeout) do
        Application.delete_env(:core, :workspace_hook_timeout_ms)
      else
        Application.put_env(:core, :workspace_hook_timeout_ms, previous_timeout)
      end
    end)

    Application.put_env(:core, :workspace_hook_timeout_ms, 10)
  ...
  test "config reads defaults for optional settings" do
    ...
    assert config.claude.mcp_config_path == nil
    assert config.claude.effort == nil
    ...
    write_workflow_file!(Workflow.workflow_file_path(),
      claude_command: "claude --verbose",
      claude_model: "claude-opus-4-6",
      claude_permission_mode: "acceptEdits",
      claude_mcp_config_path: "/etc/symphony/mcp.json",
      claude_effort: "high"
    )

    config = Config.settings!()
    assert config.claude.command == "claude --verbose"
    assert config.claude.model == "claude-opus-4-6"
    assert config.claude.permission_mode == "acceptEdits"
    assert config.claude.mcp_config_path == "/etc/symphony/mcp.json"
  ...
  test "config resolves $VAR references for env-backed secret and path values" do
    ...
    assert config.claude.command == "#{claude_bin}"
  end
