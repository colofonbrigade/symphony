defmodule Core.ObservabilityPubSubTest do
  ...
  test "broadcast_update is a no-op when pubsub is unavailable" do
    ...
    on_exit(fn ->
      if Process.whereis(Core.PubSub) == nil do
        assert {:ok, _pid} =
                 Supervisor.restart_child(Core.Supervisor, pubsub_child_id)
      end
    end)

    assert is_pid(Process.whereis(Core.PubSub))
    assert :ok = Supervisor.terminate_child(Core.Supervisor, pubsub_child_id)
    refute Process.whereis(Core.PubSub)
