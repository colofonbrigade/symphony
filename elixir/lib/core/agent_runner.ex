defmodule Core.AgentRunner do
      ...
      {:ok, workspace} ->
        ...
        try do
          with :ok <- Workspace.run_before_run_hook(workspace, issue, worker_host) do
            run_claude_turns(workspace, issue, agent_update_recipient, opts, worker_host)
          end
        after
  ...
  defp run_claude_turns(workspace, issue, agent_update_recipient, opts, worker_host) do
  ...
  defp do_run_claude_turns(claude_session, workspace, issue, agent_update_recipient, opts, issue_state_fetcher, turn_number, max_turns) do
    ...
    with {:ok, turn_result} <-
           Session.run_turn(
             ...
             prompt,
             issue,
             on_message: claude_message_handler(agent_update_recipient, issue)
           ) do
