...
polling:
  interval_ms: 5000
workspace:
  root: ${SMOKE_WORKSPACE_ROOT}
hooks:
  after_create: |
    git clone --depth 1 --branch "${SMOKE_REPO_BRANCH}" "${SMOKE_REPO_URL}" .
...
## Prerequisite: official Linear MCP server is available

The agent should be able to talk to Linear via the official Linear MCP server configured in the Claude Code session. If it is not present, stop and ask the user to configure Linear.
